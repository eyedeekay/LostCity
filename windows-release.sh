...
github-release upload --user "${GITHUB_USER}" \
    --repo "${GITHUB_REPO}" \
    --tag "${GITHUB_TAG}" \
    --label "Unciv-Over-I2P(LostCity) launcher as a Jpackage inside of an MSI package. ${msisum}" \
    --name "lostcity.msi" \
...
github-release upload --user "${GITHUB_USER}" \
    --repo "${GITHUB_REPO}" \
    --tag "${GITHUB_TAG}" \
    --label "Unciv-Over-I2P(LostCity) launcher as a Jpackage inside of an EXE package. ${exesum}" \
    --name "lostcity.exe" \
...
github-release upload --user "${GITHUB_USER}" \
    --repo "${GITHUB_REPO}" \
    --tag "${GITHUB_TAG}" \
    --label "Unciv-Over-I2P(LostCity) launcher as a Jpackage inside of an zip to be run from a directory on Windows. ${zipsum}" \
